...
class Database:
    ...
    def set(self,Data):
        ...
        for a in range(len(Data.Table)):
            ...
            self.cursor.execute("""SELECT COUNT(1) FROM `Vertretungsplan` WHERE `Klasse`='{0}' and `Stunde`='{1}' and `Date`='{date}'""".format(*Data.Table[a],Data.Name))
            if  self.cursor.fetchall()[0][0]==0:
                self.cursor.execute('''INSERT INTO `{name}` VALUES('{date}','{0}','{1}','{2}','{3}','{4}','{5}','{6}');'''.format(*Data.Table[a],Data.Name))
    ...
    def setup(self):
        self.cursor.execute('''CREATE TABLE  if not exists`Vertretungsplan` (   `Date` tinytext,`Klasse` tinytext,   `Stunde` tinytext,   `Vertreter` tinytext,   `Fach` tinytext,   `Raum` tinytext,   `Entfallener_Lehrer` tinytext,   `Entfallenes_Fach` tinytext) ENGINE=InnoDB DEFAULT CHARSET=utf8;'''.format(Data.Name))
